...
reimbursement_audit.py: 报销状态机的证据层；在同一 finance 事务里捕获状态修改、核销/撤销与删除回款级联的逐笔 before/after，批量写入只由 service 登记一份最终整库快照。
...
reports.py: 月度、预算双键、基于全部非调整流水的 0.98^days 习惯投影与 dashboard 只读聚合；不触发周期记账。习惯投影不能只看最近 500 条，否则一个仍有统计价值的旧习惯会在新流水达到阈值后突然「消失」。「本月」按记录时区判定而非宿主机器——记录时区 Asia/Shanghai、宿主 America/Los_Angeles 时，北京每月 1 号 0–8 点宿主还停在上个月，汇总/预算/趋势会整块查到上个月。所有跨账户合计（月汇总、12 个月趋势、净资产）一律按 amount_in_base_currency 结算，裸加原币会把 1000 元 + 1000 美元算成 2000。
