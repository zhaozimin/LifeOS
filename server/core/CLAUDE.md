...
httpd.py: Host/Bearer/路由/脱敏日志总边界；所有响应强制附加不可覆盖的 `frame-ancestors 'none'`、`X-Frame-Options: DENY` 与 nosniff，阻断已登录面板被第三方页面嵌入后劫持点击；响应头在写出第一个字节前统一校验可编码性与控制字符，避免「状态行已发出才失败」产出一个响应两个状态码；静态旁路后才进入认证 API。回环 Host 同时接受 127.0.0.1 与 localhost；每条路由自带请求体上限；routes_from_contract 按域 ROUTES 装配路由，契约与实现漂移即拒绝启动；未预期异常打 traceback 到 stderr。
