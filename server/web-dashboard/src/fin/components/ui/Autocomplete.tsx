/**
 * [INPUT]: 依赖 react、react-dom 的 createPortal、lucide 的 Check/ChevronsUpDown/Search、clsx、共享 components/ui/Select。
 ...
 * [POS]: 财务域独有的下拉原语；财务页面下拉统一入口。桌面统一风格、手机原生手感由 useCoarsePointer 分流。
 *   自适应判据：matchMedia('(pointer: coarse)')，可随外接鼠标/设备旋转实时切换。
 *   与共享 components/ui/Autocomplete 的分叉是行为性的：本副本展开时把活动项落在首项，
 *   共享副本落在已选项并跳过 disabled 项；TimeRangePicker 的「（无流水）」占位依赖前者，
 *   合并会改变财务侧键盘起点，故保留独立实现。
