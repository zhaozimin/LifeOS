...
FlowPage.tsx: 全局资金流桑基图与抽屉钻取；三视图 tab 仅 dual 模式显示；流水按 FULL_LEDGER_LIMIT 取整本账——桑基路径、可支撑月数与时间范围栏的可选桶全部由这份数据推导，一旦被分页截断，早期月份会连同它们的账一起从界面上消失；整页一屏装下——根容器 = 视口−顶栏−留白的纵向 flex，桑基区 flex-1 吃掉剩余高度（不随数据量变化），标题与 KPI 行同屏可见，极小窗口 560px 保底转页面滚动。（首次引导已搁置，待整套使用教程后重做。）
