...
一行说清四件事：**多少钱、什么事、走哪个账户、归了哪一类**。

```
✓ ¥38.00 午饭 · 微信支付 · 餐饮
⇄ ¥3000.00 还信用卡 · 工资卡→招商信用卡 · 转账
```

账户与分类必须在行内，因为它们恰恰是最容易被你推断错、又最该被用户一眼否决的两个字段——分类归错了统计会偏，账户记错了余额会偏，两者当时不说，事后都要靠对账才发现，而那时已经隔了很多笔。转账画出两端箭头：它唯一的风险是方向记反，只印一个账户名看不出是转出还是转入。字段缺失时该段自动省略，不印占位符。
